*** Settings ***
Documentation     This is a library for interacting with containers inside pods by "kubectl exec" on host.
...
...
Open_New_Bash_Container_Connection
    ...
    [Documentation]    Open anothe SSH connection, configure if prompt,
...
Execute_In_Container
    ...
    [Documentation]    Execute "kubectl exec" with given command and parameters on active connection, return the result.
    ...    Note this leaves SSHLibrary state with no active connection, hence Unsafe.
