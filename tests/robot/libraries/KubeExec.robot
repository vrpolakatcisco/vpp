...
...
...               In any case, it is assumed that the active SSH connection leads to host the pod is deployed on.
...
Open_New_Bash_Container_Connection
    ...
    ...    execute interactive bash on first container of \${pod_name}, read until prompt, log and return output.
    ...    Note that this puts the previously active SSH connection to background, so rember to switch later.
...
Execute_In_Container
    ...
    [Documentation]    Execute "docker exec" with given command and parameters on active connection (in parallel bash),
    ...    return the result.
