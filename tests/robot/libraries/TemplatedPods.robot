...
Subtitute_Yaml_Template
    [Arguments]    ${template}    ${name}    ${replicas}=1    ${location}=node_1
...
Deploy_Templated_Pods_And_Verify_Running
    [Arguments]    ${template_file}    ${name}    ${replicas}=1    ${location}=node_1    ${namespace}=defaut    ${options}=${EMPTY}    ${timeout}=60s    ${check_period}=5s
...
Remove_Templated_Pods_And_Verify_Removed
    [Arguments]    ${template_file}    ${name}    ${replicas}=1    ${location}=node_1    ${namespace}=default    ${options}=${EMPTY}    ${expected_rc}=0    ${ignore_stderr}=${False}    ${timeout}=60s    ${check_period}=5s
...
With_Item_Data
    ...
    ${file_path} =    BuiltIn.Set_Variable    ${CURDIR}/../resources/templates/${file_name}.yaml
