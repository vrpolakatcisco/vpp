...
Deploy_Templated_Pods_And_Verify_Running
    ...
    BuiltIn.Comment    TODO: Namespace could also be considered a field to configure in a template.
    ${template} =    OperatingSystem.Get_File    ${template_file}
    ${text} =    Subtitute_Yaml_Template    ${template}    ${name}    ${replicas}    ${location}
    BuiltIn.Run_Keyword_And_Continue    PodManagement.Deploy_Text_Pods_And_Verify_Running    ${text}    ${name}    ${replicas}    ${namespace}    ${options}    ${timeout}    ${check_period}
...
For_Every_Item
    ...
    \    With_Item_Data    ${item}    ${keyword}    @{args}    &{kwargs}
