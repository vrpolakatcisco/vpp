...
...               The exception is Deploy_Text_Pods_And_Verify_Running
...               which calls StatefulSetup.Store_Pods_Info to set variables tracked by StatefulSetup.
...
...
Execute_Kubectl_Command
    [Arguments]    ${subcommand}    @{args}    &{kwargs}
    [Documentation]    Switch to master SSH connection, execute kubectl, switch back to previously active connection, return output.
    Builtin.Log_Many    ${subcommand}    ${args}    ${kwargs}
    BuiltIn.Run_Keyword_And_Return    KubeManagement.On_Master_Connection    SshCommons.Execute_Command_And_Log    kubectl ${subcommand}    @{args}    &{kwargs}

Get_Logs
    [Arguments]    ${pod_name}    ${container}=${EMPTY}    ${namespace}=${EMPTY}
    [Documentation]    Execute "kubectl logs" with given parameters on master and return the result.
    ...
    BuiltIn.Run_Keyword_And_Return    Execute_Kubectl_Command    logs${nsparam} ${pod_name}${cntparam}

Describe_Pod
    [Arguments]    ${pod_name}
    [Documentation]    Execute "kubectl describe pod" with given \${pod_name} on master, parse,
    ...    log and return the parsed details.
    BuiltIn.Log_Many    ${pod_name}
    ${output} =    Execute_Kubectl_Command    describe pod ${pod_name}
...
Get_Pods_Full
    ...
    [Documentation]    Execute "kubectl get pods" with optional arguments for given \${namespace} and \${options} on master,
    ...
    ${status}    ${message} =    BuiltIn.Run_Keyword_And_Ignore_Error    Execute_Kubectl_Command    get pods${spaced_namespace}${spaced_options}
...
Check_Pods_Running
    ...
    ...    And alternative of having WUKS inside FOR could take too long to finish in case of large failure.
    BuiltIn.Log_Many    ${pod_names}    ${namespace}    ${options}
    : FOR    ${name}    IN    ${pod_names}
    \    Check_Pod_Running_Containers_Ready    ${name}    ${namespace}

Wait_For_Pods_Running
    [Arguments]    ${pod_names}    ${namespace}=default    ${options}=${EMPTY}    ${timeout}=60s    ${check_period}=5s
...
Deploy_Text_Pods_And_Verify_Running
    [Arguments]    ${pod_text}    ${pod_prefix}    ${multiplicity}=1    ${location}=${EMPTY}    ${namespace}=defaut    ${options}=${EMPTY}    ${timeout}=60s    ${check_period}=5s
...
Deploy_File_Pods_And_Verify_Running
    [Arguments]    ${pod_file}    ${pod_prefix}    ${multiplicity}=1    ${location}=${EMPTY}    ${namespace}=defaut    ${options}=${EMPTY}    ${timeout}=60s    ${check_period}=5s
...
Remove_Text_Pods_And_Verify_Removed
    [Arguments]    ${pod_text}    ${pod_prefix}    ${namespace}=default    ${options}=${EMPTY}    ${expected_rc}=0    ${ignore_stderr}=${False}    ${timeout}=60s    ${check_period}=5s
...
Remove_File_Pods_And_Verify_Removed
    [Arguments]    ${pod_file}    ${pod_prefix}    ${namespace}=default    ${options}=${EMPTY}    ${expected_rc}=0    ${ignore_stderr}=${False}    ${timeout}=60s    ${check_period}=5s
