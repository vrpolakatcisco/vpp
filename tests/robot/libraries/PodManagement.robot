...
Deploy_Text_Pods_And_Verify_Running
    [Arguments]    ${pod_text}    ${pod_prefix}    ${multiplicity}=1    ${namespace}=defaut    ${options}=${EMPTY}    ${timeout}=30s    ${check_period}=5s
    ...
    Builtin.Log_Many    ${pod_text}    ${pod_prefix}    ${multiplicity}    ${namespace}    ${options}    ${timeout}    ${check_period}
...
Deploy_File_Pods_And_Verify_Running
    ...
    BuiltIn.Run_Keyword_And_Continue    Deploy_Text_Pods_And_Verify_Running    ${pod_text}    ${pod_prefix}    ${multiplicity}    ${namespace}    ${options}    ${timeout}    ${check_period}
