...
Switch_Execute_And_Log_To_File
    [Arguments]    ${ssh_session}    ${command}    ${expected_rc}=0    ${ignore_stderr}=${False}    ${log_stdout}=${False}
    [Documentation]    Call Switch_And_Execute_Command without logging stdout, put stdout into a file. Return stdout.
    ...    To distinguish separate invocations, suite name, test name, session alias
    ...    and full command are used to construct file name.
    BuiltIn.Log_Many    ${ssh_session}    ${command}    ${expected_rc}    ${ignore_stderr}    ${log_stdout}
    ${stdout} =    Switch_And_Execute_Command    ${ssh_session}    ${command}    ${expected_rc}    ${ignore_stderr}    ${log_stdout}
    ...
    ${session} =    SSHLibrary.Get_Session
    ${filename} =    ${TEST_NAME}__${SUITE_NAME}__${session.alias}__${underlined_command}.log
    BuiltIn.Log    ${filename}
    OperatingSystem.Create_File    ${RESULTS}${/}${filename}    ${stdout}
    [Return]    ${stdout}

Switch_And_Execute_Command
    [Arguments]    ${ssh_session}    ${command}    ${expected_rc}=0    ${ignore_stderr}=${False}    ${log_stdout}=${True}
    [Documentation]    Switch to \${ssh_session}, and continue with Execute_Command_And_Log.
    BuiltIn.Log_Many    ${ssh_session}    ${command}    ${expected_rc}    ${ignore_stderr}    ${log_stdout}
    SSHLibrary.Switch_Connection    ${ssh_session}
    BuiltIn.Run_Keyword_And_Return    Execute_Command_And_Log    ${command}    expected_rc=${expected_rc}    ignore_stderr=${ignore_stderr}    log_stdout=${log_stdout}

Execute_Command_And_Log
    [Arguments]    ${command}    ${expected_rc}=0    ${ignore_stderr}=${False}    ${log_stdout}=${True}
    ...
    BuiltIn.Log_Many    ${command}    ${expected_rc}    ${ignore_stderr}    ${log_stdout}
    ...
    BuiltIn.Run_Keyword_If    """${log_stdout}""" == "True"    BuiltIn.Log    ${stdout}
    BuiltIn.Log    ${stderr}
    BuiltIn.Log    ${rc}
