...
...               \${container_aliases} List of SSH connection aliases opened towards containers in pod.
...
Get_Host_Ip_By_Location
    ...
    ${host_ip} =    NamedVms.Get_Host_Ip_For_Index    ${host_index}
    BuiltIn.Log    ${host_Ip}
    [Return]    ${host_ip}
...
Deploy_Single_Pod_For_Type
    [Argument]    ${type}
    [Documentation]    Proceed with Deploy_Single_* keyword for type.
    ...    DEPRECATED: Migrate to Setup_Suite_With_Pod_Set
    BuiltIn.Log_Many    ${type}
    BuiltIn.Run_Keyword_And_Return    NamedPods.Deploy_Single_${type}_And_Verify_Running
...
Deploy_Multiple_Pod_For_Type
    ...
    BuiltIn.Run_Keyword_And_Return    NamedPods.Deploy_Multiple_${type}_And_Verify_Running
...
Setup_Suite_With_Pod_Set_Text
    ...
    ${nr_nodes} =    Collections.Get_From_Dictionary    &{object}    nodes
...
Open_Single_Container_Connection_For_Type
    ...
    BuiltIn.Run_Keyword    KubeExec.Open_New_Bash_Container_Connection    ${type}    ${pod_name}
...
Open_Multiple_Container_Connections_For_Type
    ...
    \    BuiltIn.Run_Keyword    KubeExec.Open_New_Bash_Container_Connection    ${type}_${index}    ${pod_name}
